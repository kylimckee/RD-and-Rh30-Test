...
```bash
cd /data/mckeeka/bulkRNA_RMS
sbatch --cpus-per-task=4 --mem=16G --time=08:00:00 \--wrap "snakemake -s CutAdapt_pipeline.smk -j 4"
```
...
SAMPLES = sorted(set(glob_wildcards(
    "run_bulkRNA/trimmed_FASTQ/{sample}.fastq.R1.trimmed.gz"
).sample))

rule all:
    input:
        "run_bulkRNA/clean_FASTQ/ReadCounts_output/read_summary.csv",
        expand("run_bulkRNA/trimmed_FASTQ/{sample}.fastq.R{read}.count.txt",
               sample=SAMPLES, read=[1, 2]),
        expand("run_bulkRNA/clean_FASTQ/kraken2_output/{sample}.fastq.R{read}.kraken.count.txt",
               sample=SAMPLES, read=[1, 2]),
        expand("run_bulkRNA/clean_FASTQ/{sample}.fastq.R{read}.clean.count.txt",
               sample=SAMPLES, read=[1, 2])
...
rule count_trimmed_reads:
    ...
    shell:
        ...
        if gzip -t {input} 2>/dev/null; then
            gzip -cd {input} | awk 'END {{print NR/4}}' > {output}
        else
            awk 'END {{print NR/4}}' {input} > {output}
        fi
        """
...
rule summarize_read_counts:
    input:
        TRIMMED = expand("run_bulkRNA/trimmed_FASTQ/{sample}.fastq.R{read}.count.txt",
                         sample=SAMPLES, read=[1, 2]),
        KRAKEN = expand("run_bulkRNA/clean_FASTQ/kraken2_output/{sample}.fastq.R{read}.kraken.count.txt",
                        sample=SAMPLES, read=[1, 2]),
        BOWTIE = expand("run_bulkRNA/clean_FASTQ/{sample}.fastq.R{read}.clean.count.txt",
                        sample=SAMPLES, read=[1, 2])
    output:
        "run_bulkRNA/clean_FASTQ/ReadCounts_output/read_summary.csv"
    run:
        ...
        for sample in SAMPLES:
            ...
            for step, path_template in [
                ...
                for read in [1, 2]:
                    ...
                    row[f"{step}_{read}"] = value
                    counts.append(value)

                row[f"{step}_TOTAL"] = sum(counts)
...
```bash
# Rename the FASTQ files so they are compatible with the pipeline
cd /data/mckeeka/bulkRNA_RMS/riboseq

for f in *_S*_R*_001.fastq.gz; do
base=$(basename "$f")

Sample=$(echo "$base" | awk -F'_' '{print $2}')  # RD
Read=$(echo "$base" | awk -F'_' '{print $4}')  # R1 or R2

mv "$f" "${Sample}_RIBO_${Read}.fastq.gz"
done

# Create run_riboseq configuration file
nano run_riboseq.sh
...
export NXF_APPTAINER_CACHEDIR=/vf/users/mckeeka/bulkRNA_RMS/work/singularity
...
nextflow run nf-core/riboseq \
  ...
  -profile apptainer \
  -c apptainer.config \
  --input riboseq_samplesheet.csv \
  --outdir results \
  --fasta $GENOME_FASTA \
  --gtf $GTF \
  -resume

# Create apptainer configuration file
nano apptainer.config

apptainer {
  enabled = true
  libraryDir = "/vf/users/mckeeka/bulkRNA_RMS/work/singularity"
  cacheDir   = "/vf/users/mckeeka/bulkRNA_RMS/work/singularity"
  pullTimeout = '20 min'
}

#Run pipeline
sbatch --cpus-per-task=24 --mem=96G --time=3-00:00:00 run_riboseq.sh
```
